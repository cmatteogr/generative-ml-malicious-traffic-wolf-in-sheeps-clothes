...
def train(traffic_data_filepath: str, results_folder_path: str, discriminator_filepath: str, train_size_percentage=0.8, batch_size=1024):
    """
    Beta-VAE-GAN training

    :param traffic_data_filepath: Traffic dataset
    :param results_folder_path: Folder path where save the results
    :param discriminator_filepath: Discriminator, Classifier model filepath
    ...
    train_subset, val_subset = random_split(tensor_data, [train_size, val_size])

    # Create DataLoader
    train_loader = DataLoader(train_subset, batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(val_subset, batch_size=batch_size, shuffle=False)

    # get the labels, correspond to the same train and validation datasets
    train_indices = train_subset.indices
    val_indices = val_subset.indices
    train_labels = label.iloc[train_indices]
    val_labels = label.iloc[val_indices]
    ...
    def train_model(trial):
        ...
        generative_model: VAE = VAE(input_dim=n_features, latent_dim=latent_dim, hidden_dim=hidden_dim).to(device)
        ...
        for epoch in range(num_epochs):
            ...
            for label, data in zip(train_labels, train_loader):
                # Forward pass
                data = data.to(device)
                optimizer.zero_grad()

                # use the discriminator to classify traffic
                pred_real_label = discriminator_model.predict(data)
                # use the generative model to classify the traffic
                data_reconstructed = generative_model.reconstruct_x(data)
                pred_fake_label = discriminator_model.predict(data_reconstructed)
                # compare real and fake labels,
                are_equal_items = (pred_real_label == pred_fake_label)
                fool_fail_score_batch = 1 - np.mean(are_equal_items.astype(float))

                # Use the B-VAE to calculate loss function
                reconstruction_loss_value, kl_value = generative_model(data, reduction='avg')
                kl_value = kl_value * kl_beta
                # Add the fool fail score to integrate the discriminator, and then the GAN
                loss = reconstruction_loss_value + kl_value + fool_fail_score_batch
            ...
            val_loss_accum = 0.0
            with torch.no_grad():
                for label, data in zip(val_labels, val_loader):
                    data = data.to(device)
                    reconstruction_loss_value, kl_value = generative_model(data, reduction='avg')
                    kl_value = kl_value * kl_beta
