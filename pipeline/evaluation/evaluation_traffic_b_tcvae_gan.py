...
def evaluation(model_filepath: str, data_test_filepath: str, results_folder_path: str,
    ...
    traffic_df = pd.read_csv(os.path.join(results_folder_path, data_test_filepath))
    ...
    initial_type = [('input', FloatTensorType([None, traffic_df.shape[1]]))]
    # convert to ONNX and save
    model_onnx_filepath = os.path.join(results_folder_path, 'xgb_server_traffic_classifier.onnx')
    #onnx_model = convert_xgboost_booster(loaded_model, "name", initial_types=initial_type, target_opset=12)
